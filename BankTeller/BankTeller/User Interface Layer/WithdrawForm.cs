            ...
            if (!string.IsNullOrEmpty(txtNum.Text))
            {
                ...
                if (withdrawAmount > 0)
                {
                    if (AccountEntity.Balance >= withdrawAmount)
                    {
                        ...
                        DialogResult result = MessageBox.Show("Withdraw transaction successful. New balance: " + AccountEntity.Balance + ". Do you wish to do another transaction?", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
